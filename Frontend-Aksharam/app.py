...
if st.button("Translate"):
    if not english_text.strip():
        ...
        output_text = "Translation will appear here once model is integrated."
        st.success("Output generated (placeholder).")
...
#     if device == "cuda"
#     else '<span class="badge-cpu">○ CPU</span>'
# )
# st.markdown(f"<div style='text-align:right;margin-top:-0.5rem;margin-bottom:1rem'>{badge}</div>",
#             unsafe_allow_html=True)

# ── Input card ───────────────────────────────────────────────────────────────
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="area-label">English input</div>', unsafe_allow_html=True)
english_text = st.text_area(
    label="",
    height=140,
    placeholder="Type or paste your English text here…",
...
# ── Output card ──────────────────────────────────────────────────────────────
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="area-label">Malayalam output</div>', unsafe_allow_html=True)
output_slot = st.empty()
output_slot.markdown('<div class="ml-placeholder">Translation appears here…</div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
if translate:
    if not english_text.strip():
        ...
        with st.spinner("Translating…"):
            ...
            #     )
            translation = "Translation will appear here once model is integrated."

        output_slot.markdown(
            f'<div class="ml-output">{translation}</div>',
            unsafe_allow_html=True,
        )
...
<div style="text-align:center;margin-top:3rem;font-family:'Space Mono',monospace;
            font-size:0.65rem;color:#2a2a35;letter-spacing:0.12em;">
  MODEL · facebook/mbart-large-50 · FINE-TUNED
</div>
""", unsafe_allow_html=True)